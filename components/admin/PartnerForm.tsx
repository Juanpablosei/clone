...
export default function PartnerForm({
  ...
  const [originalImage, setOriginalImage] = useState<string | null>(partner?.image || null);

  // Cargar datos del partner cuando se pasa como prop
  useEffect(() => {
    if (partner) {
      setFormData({
        id: partner.id,
      ...
      setOriginalImage(partner.image);
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log("✅ Partner saved successfully:", responseData);
        if (!formData.id) {
          // Solo limpiar si es un nuevo partner
          setFormData({
            id: "",
            name: "",
            image: "",
        ...
        setError(data.error || (formData.id ? "Error updating partner" : "Error creating partner"));
      }
    } catch (err: any) {
