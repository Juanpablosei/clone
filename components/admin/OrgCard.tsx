...
export default function OrgCard({ item, onEdit, onDelete }: OrgCardProps) {
    ...
    <div className="group relative flex h-full flex-col overflow-hidden rounded-2xl border border-[var(--admin-border)] bg-[var(--admin-surface)] shadow-sm transition hover:shadow-lg lg:flex-row">
      ...
      <div className="flex flex-1 flex-col p-5 lg:w-2/3">
        ...
        <div className="mb-3 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-[var(--admin-text)]">{item.name}</h3>
        </div>
        ...
        <div className="mb-4 flex-1">
          {item.description ? (
            <p className="line-clamp-4 text-sm leading-relaxed text-[var(--admin-text-muted)]">{item.description}</p>
          ) : (
            <p className="text-sm italic text-[var(--admin-text-muted)] opacity-60">No description available</p>
          )}
        </div>
